...
class Blockchain {
    ...
    init(peers) {
        // If this is a new node download the genesis block from a peer
        if (this.blocks.length == 0 && peers.length > 0){
        	const URL = `${peers[0].url}/blockchain/blocks/0`;
        	console.info(`New Node Getting Blockchain From: ${URL}`);
        	return superagent
            		.get(URL)
            		.then((res) => {
                		this.blocks.push(Block.fromJson(res.body));
            		})
        }
        // Create the genesis block if the blockhain is empty
