...
class Miner {
    ...
    mine(rewardAddress, feeAddress) {
    	//Let's make sure that the rewardAddress and the feeAddress are in our  blockchain
    	var rflag = 0, fflag = 0;
    	var a, alength;
    	
    	this.operator = Operator.blockchainOnly(this.dbName, this.blockchain);
    	let w = this.operator.getWallets();
        let length = w.length;
        for (var i = 0; i < length; i++){
        	a  = w[i].getAddresses();
               	alength = a.length;
               	for (var j = 0; j < alength; j++){
               		if (rewardAddress == a[j]){
               			rflag = 1;
               		}
               		if (feeAddress == a[j]){
               			fflag = 1;
        ...
        if (rflag == 0 || fflag == 0) throw new ArgumentError('Reward/Fee Address not found in blockchain');
