...
class Node {
    constructor(host, port, peers, blockchain, dbName) {
        this.host = host;
        this.port = port;
        this.peers = [];
        this.dbName = dbName
        this.peerDB = new Db('data/' + dbName + '/' + PEERS_FILE, new Peers());
    ...
    connectToPeers(newPeers) {
        ...
        //Add to peers database
        
        fs.ensureDirSync(path.dirname('data/' + this.dbName + '/peers.json'));
        fs.writeFileSync('data/' + this.dbName + '/peers.json', JSON.stringify(this.peers));
